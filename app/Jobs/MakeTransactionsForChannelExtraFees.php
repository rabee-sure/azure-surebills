...
class MakeTransactionsForChannelExtraFees
{
    ...
    public function handle()
    {
        ...
        if(isset($this->bill->application) && isset($channel)){
            ...
            $percentage = $this->bill->getPercentage($this->log);
            $channel_extra_amount_fees = $this->bill->channel_extra_amount * ($percentage / 100);
            $channel_extra_amount_vat = $this->bill->channel_extra_amount * (Transaction::VAT_PERCENTAGE / 100);

            $fee_trans = new Transaction;
            $fee_trans->user_id     = $channel->user_id;
            $fee_trans->bill_id     = $this->bill->id;
            $fee_trans->type        = 'credit';
            $fee_trans->amount      = $this->bill->channel_extra_amount;
