...
class MakeReverseTransactionsForOwner
{
    ...
    {
        $prefix = $this->bill->debit_note_bill_id ? 'DN' : 'Bill';
        ...
        $transaction->amount      = ($this->bill->fixed_total - $this->bill->channel_extra_amount - $this->bill->channel_extra_vat);
        $transaction->reference   = $this->bill->number;
        $transaction->description = $prefix.''.$this->bill->number .' '.$dash.' '. $this->bill->customer_name.' - reversed for payment failed';
        ...
        $transaction->reference   = $this->bill->number;
        $transaction->description = 'Fee - Reversed Transaction Processing';
        ...
        $transaction->reference   = $this->bill->number;
        $transaction->description = 'VAT - Reversed Transaction Processing';
