...
class TransferTransactionsSettled extends Command
{
    ...
    public function handle()
    {
        ...
        $this->info("completed transfers ids: {$completed_transfers}"); 
        ...
        if(!empty($completed_transfers)){
            ...
            $this->info("completed transfers transactions ids: {$completed_transfers_transactions}");
        }
        ...
        $this->info("pending transfers ids: {$pending_transfers}");
        ...
        if(!empty($pending_transfers)){
            ...
            $this->info("pending transfers transactions ids: {$pending_transfers_transactions}");
        }

        // Unsettled all user transactions
        Transaction::where('user_id', $user_id)->update(['pending_settled' => false, 'settled' => false]);
        $this->info("all user transactions unsettled");
        ...
        if(!empty($completed_transfers_transactions)){
            ...
            $this->info("Settled completed transfer transactions");
        }
        ...
        if(!empty($pending_transfers_transactions)){
            ...
            $this->info("Pending settled pending and send to sps transfers transactions");
        }
