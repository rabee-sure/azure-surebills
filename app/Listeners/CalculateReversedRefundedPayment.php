...
use App\Jobs\RefundReverseTransactionsForChannel;
use App\Jobs\RefundReverseTransactionsForOwner;
use App\Jobs\RefundReverseTransactionsForSureBills;
use App\Models\Transaction;

class CalculateReversedRefundedPayment
{
    /**
     ...
     * @return void
     */
    public function handle(BillRefundedReversed $event)
    {
        $bill = $event->bill;
        ...
        if($bill && $payment_log){
            ...
            RefundReverseTransactionsForOwner::dispatch($bill, $payment_log);

            // refund fees
            if($bill->user->able_refund_with_fees){
                //Refund Transactions For Channel
                RefundReverseTransactionsForChannel::dispatch($bill, $payment_log);

                //Refund Transactions For SureBills
                RefundReverseTransactionsForSureBills::dispatch($bill, $payment_log);
            }
        }
    }
