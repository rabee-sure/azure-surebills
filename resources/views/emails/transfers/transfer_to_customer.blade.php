      ...
      <tr>
        <td style="background-color: #edf2f7;padding: 25px;">
          <div style="background-color: #ffffff;padding: 30px;border-radius: 20px;box-shadow: 0 0.125rem 0.25rem rgba(0,0,0,0.08);">
            @if($transfer->user->logo)
              <div class="logo" style="display: block;text-align: center;margin-bottom: 1rem;">
                <img src="{{ $transfer->user->logo_url }}" alt="logo">
              </div><!-- logo -->
            @endif
            <b style="text-align:center;color:#000;display: block;margin-bottom: 1rem;">{{ $transfer->user->business_address }}</b>
            <ul style="color:#000;list-style: none;margin: 0;padding: 0;">
              <li style="font-size: 16px;margin-bottom: 10px;color: #777777;">
                
              </li>
              <li style="font-size: 16px;margin-bottom: 10px;color: #777777;">
                <!-- {{ __('An amount of :amount has been transferred to your bank account number #:bank_number. Operation number #:number', [
                'amount' => $transfer->net_amount,
                'number' => $transfer->id,
                'bank_number' => $transfer->iban_number,
                'bank_name' => $transfer->bank->name,
                ], 'ar') 
                }} -->
                تم تحويل مبلغ 
                <b dir="ltr" style="color: #000">{{$transfer->net_amount}}</b> 
                لحسابك البنكي 
                <b dir="ltr" style="color: #000">#{{$transfer->iban_number}}</b>
                <span> - </span>
                برقم العملية
                <b dir="ltr" style="color: #000">#{{$transfer->id}}</b>
              </li>
              <li style="font-size: 16px;margin-bottom: 10px;color: #777777;">
                {{__('Transfer Amount Due', [], 'ar')}} : <b dir="ltr" style="color: #000000;">{{$transfer->net_amount}}</b>
              </li>
              <li style="font-size: 16px;margin-bottom: 10px;color: #777777;">
                {{__('Transfer Fees', [], 'ar')}} : <b dir="ltr" style="color: #000000;">{{$transfer->transfer_fees}}</b>
              </li>
              <li style="font-size: 16px;margin-bottom: 10px;color: #777777;">
                {{__('Transfer Amount', [], 'ar')}} : <b dir="ltr" style="color: #000000;">{{$transfer->amount}}</b>
              </li>
            </ul>
