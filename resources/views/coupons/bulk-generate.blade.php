...
@section('content')
  ...
  <section id="couponsBulkGeneratePage">
    ...
    <div class="title mb-4">
      <h1 class="d-block fw-bold m-0 fs-5">{{ __('Generate Coupon Codes')}}</h1>
    ...
    <div class="bulkGenerateArea bg-white shadow-sm rounded-3 p-4 mb-3">
      <form method="POST" action="{{ route('coupons.store-bulk-generate', $coupon->id) }}" id="bulk_generate_form">
        ...
        <div class="form-group mb-3">
          <label for="count" class="d-block mb-1">{{ __('Number of Codes to Generate') }} <span class="requirement text-danger">*</span></label>
          <input name="count" 
                 type="number" 
                 min="1"
                 max="10000"
        ...
        <div class="form-group mb-3">
          <label for="pattern" class="d-block mb-1">{{ __('Code Pattern') }}</label>
          <input name="pattern" 
                 type="text" 
                 class="form-control shadow-none bg-white border w-100 rounded-3 text-body" 
                 id="pattern" 
