...
<div class="billPrint">
  ...
  <div id="status" class="my-3">
    @if($bill->status == 'expired')
      <div class="alertMsg text-center fw-bold expired"> {{ __('this bill has been expired', ['number' => $bill->number ]) }}</div>
    ...
    @elseif($bill->status == 'paid_cash')
      <div class="alertMsg text-center fw-bold paid"> {{ __('this bill has been Paid Cash successfully', ['number' => $bill->number ]) }}</div>
    @elseif($bill->status == 'paid_bank_transfer')
      <div class="alertMsg text-center fw-bold paid"> {{ __('this bill has been Paid Bank Transfer successfully', ['number' => $bill->number ]) }}</div>
    @elseif($bill->status == 'canceled')
      <div class="alertMsg text-center fw-bold canceled"> {{ __('this bill has been canceled', ['number' => $bill->number ]) }}</div>
    @elseif($bill->status == 'failed')
      <div class="alertMsg text-center fw-bold canceled"> {{ __('this bill has been failed', ['number' => $bill->number ]) }}</div>
    @elseif(in_array($bill->status, ['refunded', 'refunded_cash', 'refunded_bank_transfer']))
      <div class="alertMsg text-center fw-bold refunded"> {{ __('this bill has been refunded', ['number' => $bill->number ]) }}</div>
    @endif
  </div><!-- status -->
