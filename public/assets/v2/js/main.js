...
(function () {
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    // Theme toggle button: one click switches between light and dark (same mechanism as dropdown)
    ...
    if (themeToggleBtn) {
      themeToggleBtn.addEventListener('click', () => {
        ...
        window.Helpers.setStoredTheme(templateName, nextTheme);
        window.Helpers.setTheme(nextTheme);
        window.Helpers.showActiveTheme(nextTheme, true);
        window.Helpers.syncCustomOptions(nextTheme);
        const semiDarkL = document.querySelector('.template-customizer-semiDark');
        if (semiDarkL) {
          if (nextTheme === 'dark') {
            semiDarkL.classList.add('d-none');
        ...
        window.Helpers.switchImage(nextTheme);
      });
    }
  });
